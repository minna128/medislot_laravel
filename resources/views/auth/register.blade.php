<x-guest-layout>
    <div style="min-height:100vh; display:grid; grid-template-columns:2fr 3fr;">
        ...
        <div style="background:#0f172a; padding:48px; display:flex; flex-direction:column; justify-content:center; position:relative; overflow:hidden;">
            ...
            <div style="position:relative; z-index:1;">
                ...
                <h2 style="font-size:32px; font-weight:800; color:white; line-height:1.2; margin-bottom:12px;">
                    Join MediSlot<br>Today
                </h2>
                <p style="font-size:14px; color:rgba(255,255,255,0.6); line-height:1.8; margin-bottom:36px;">
                    Join thousands of patients who trust MediSlot for their healthcare needs.
                </p>
                <div style="display:flex; flex-direction:column; gap:12px;">
                    @foreach([
                        ['icon'=>'fa-user-doctor', 'text'=>'Access to qualified doctors'],
                        ['icon'=>'fa-hospital', 'text'=>'Multiple clinic locations'],
                        ['icon'=>'fa-clock', 'text'=>'Flexible appointment slots'],
                    ] as $f)
        ...
        {{-- Right Panel --}}
        <div style="background:white; padding:48px; display:flex; flex-direction:column; justify-content:center; align-items:center; overflow-y:auto;">
            <div style="width:100%; max-width:400px;">
                <h2 style="font-size:26px; font-weight:800; color:#0f172a; margin-bottom:6px;">Create Account</h2>
                <p style="font-size:14px; color:#6b7280; margin-bottom:32px;">Sign up for a free MediSlot account</p>

                <form method="POST" action="{{ route('register') }}">
                    @csrf

                    <div style="margin-bottom:16px;">
                        <label style="display:block; font-size:11px; font-weight:700; color:#374151; letter-spacing:0.5px; text-transform:uppercase; margin-bottom:6px;">Full Name</label>
                    ...
                    <div style="margin-bottom:24px;">
                        <label style="display:block; font-size:11px; font-weight:700; color:#374151; letter-spacing:0.5px; text-transform:uppercase; margin-bottom:6px;">Confirm Password</label>
                    ...
                    @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                        <div style="margin-bottom:20px;">
                            <label style="display:flex; align-items:flex-start; gap:10px; cursor:pointer;">
                                <input type="checkbox" name="terms" style="width:15px; height:15px; accent-color:#0d9488; margin-top:2px; flex-shrink:0;">
                                <span style="font-size:13px; color:#6b7280; line-height:1.5;">
                                    I agree to the
                                    <a href="{{ route('terms.show') }}" style="color:#0d9488; font-weight:600; text-decoration:none;">Terms of Service</a>
                                    and
                                    <a href="{{ route('policy.show') }}" style="color:#0d9488; font-weight:600; text-decoration:none;">Privacy Policy</a>
                                </span>
                            </label>
                            @error('terms') <p style="color:#dc2626; font-size:12px; margin-top:4px;">{{ $message }}</p> @enderror
                        </div>
                    @endif
