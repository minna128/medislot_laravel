<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight" style="color:white;">Manage Doctors</h2>
    </x-slot>

    <div style="max-width:1200px; margin:0 auto; padding:32px 2rem;">

        @if(session('success'))
            <div style="background:#dcfce7;color:#15803d; padding:12px 20px; border-radius:10px; margin-bottom:20px; font-size:14px; font-weight:600;">
                ✓ {{ session('success') }}
            </div>
        @endif
        ...
        <div style="background:white; border-radius:14px; padding:24px; border:1px solid #ccfbf1; margin-bottom:24px;">
            <h3 style="font-size:15px; font-weight:700; color:#0f172a; margin-bottom:16px;">🔍 Live Doctor Search</h3>
            @livewire('doctor-search')
        </div>
        ...
        <div style="background:white; border-radius:14px; padding:28px; border:1px solid #ccfbf1; box-shadow:0 2px 8px rgba(13,148,136,0.08);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
                <h3 style="font-size:18px; font-weight:700; color:#0f172a;">All Doctors</h3>
                <a href="{{ route('admin.doctor.create') }}"
                   style="display:inline-flex; align-items:center; gap:8px; background:#0d9488; color:white; padding:10px 20px; border-radius:8px; font-size:14px; font-weight:600; text-decoration:none;">
                ...
                <table style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr style="border-bottom:2px solid #f0fdfa;">
                            ...
                            <th style="text-align:left; padding:12px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:1px;">Specialization</th>
                            <th style="text-align:left; padding:12px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:1px;">Phone</th>
                            <th style="text-align:left; padding:12px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:1px;">Availability</th>
...
"
                                    >
                                    <span style="font-size:14px; font-weight:600; color:#0f172a;">Dr. {{ $doctor->user->name }}</span>
                                </div>
                            </td>
                            <td style="padding:14px 16px; font-size:14px; color:#0d9488; font-weight:500;">{{ $doctor->specialization ?? 'General' }}</td>
                            <td style="padding:14px 16px; font-size:14px; color:#6b7280;">{{ $doctor->phone ?? 'N/A' }}</td>
                            <td style="padding:14px 16px; font-size:14px; color:#6b7280;">{{ $doctor->availability ?? 'N/A' }}</td>
                            <td style="padding:14px 16px;">
                                <span style="padding:4px 12px; border-radius:50px; font-size:12px; font-weight:600; {{ ($doctor->status ?? 'active') === 'active' ? 'background:#dcfce7; color:#15803d;' : 'background:#fee2e2; color:#dc2626;' }}">
...
">
    ...
    <form method="POST"
          action="{{ route('admin.doctor.toggle', $doctor->id) }}"
          style="display:inline;">

        @csrf

        <button style="
            background: {{ ($doctor->status ?? 'active') === 'active' ? '#fef3c7' : '#dcfce7' }};
            color: {{ ($doctor->status ?? 'active') === 'active' ? '#b45309' : '#15803d' }};
            border: none; 
            padding: 8px 14px;
            border-radius: 10px;
            font-size: 12px;
        ...
        ">
                    ...
                    <i class="fa-solid {{ ($doctor->status ?? 'active') === 'active'
                        ? 'fa-pause'
                        : 'fa-play' }}"
            ...
            <form method="POST"
                action="{{ route('admin.doctor.delete', $doctor->id) }}"
                style="display:inline;"
