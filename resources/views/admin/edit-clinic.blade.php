<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight" style="color:white;">Edit Clinic</h2>
    </x-slot>
    ...
    <div style="max-width:800px; margin:0 auto; padding:32px 2rem;">
        <div style="background:white; border-radius:14px; overflow:hidden; border:1px solid #ccfbf1; box-shadow:0 2px 8px rgba(13,148,136,0.08);">
            <div style="background:linear-gradient(135deg,#0f172a,#0d9488); padding:24px 28px; display:flex; align-items:center; gap:16px;">
                ...
                <div>
                    <h2 style="font-size:20px; font-weight:800; color:white;">Edit Clinic</h2>
                    <p style="color:#2dd4bf; font-size:13px;">{{ $clinic->name }}</p>
                </div>
            </div>
                ...
                <form method="POST" action="{{ route('admin.clinic.update', $clinic->id) }}">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                        ...
                        <div>
                            ...
                            <input type="text" name="hours" value="{{ old('hours', $clinic->hours) }}"
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <textarea name="description" rows="3"
                                  style="width:100%; padding:11px 14px; border:1.5px solid #e5e7eb; border-radius:8px; font-size:14px; color:#0f172a; outline:none; font-family:inherit; resize:none;">{{ old('description', $clinic->description) }}</textarea>
                    </div>
                        ...
                        <div style="display:grid; grid-template-columns:repeat(2,1fr); gap:8px;">
                            ...
                            <label style="display:flex; align-items:center; gap:10px; padding:10px 14px; background:#f8fafc; border:1.5px solid #e5e7eb; border-radius:8px; cursor:pointer;">
                                <input type="checkbox" name="doctors[]" value="{{ $doctor->id }}"
                                       {{ $clinic->doctors->contains($doctor->id) ? 'checked' : '' }}
